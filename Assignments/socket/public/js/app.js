...
function createGame() {
    ...
    if (tag.value == "")
        ...
        gameStatus.me = tag.value;
        gameStatus.player = 1;
        gameStatus.otherPlayer = 2;
        socket.emit("createGame", gameStatus);
    }
...
function joinGame() {
    ...
    if (tag.value == "") {
        ...
        gameStatus.me = tag.value;
        gameStatus.player = 2;
        gameStatus.otherPlayer = 1;
        socket.emit("joinGame", gameStatus);
    }
...
socket.on("updateGame", (msg) => {
    console.log(msg);
    const squares = document.querySelectorAll('.game-board div');
    squares[msg.index].classList.add('taken');
    squares[msg.index].classList.add(msg.color);
    gameStatus.turn = msg.turn;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (var i = 0, len = squares.length; i < len; i++)
        ...
        //add an on-click to each square in your grid
        squares[i].onclick = function() {
            if (gameStatus.play == false)
                return;
            console.log("ssss", squares[index + 7])
                //if the square below your current square is taken, you can go ontop of it
            if (squares[index + 7].classList.contains('taken')) {
                if (gameStatus.turn == 1 && gameStatus.player == 1) {
                    squares[index].classList.add('taken');
                    squares[index].classList.add('player-one');

                    //change the player
                    gameStatus.turn = 2;
                    displayCurrentPlayer.innerHTML = gameStatus.turn;
                    gameStatus.index = index;
                    gameStatus.color = "player-one";
                    socket.emit("updateGame", gameStatus);

                } else if (gameStatus.turn == 2 && gameStatus.player == 2) {
                    squares[index].classList.add('taken');
